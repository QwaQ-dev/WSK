function fastestRunners(runners) {
    const formattedRunners = runners.map(runner => {
        const paces = runner.paces.map(pace => paceToSeconds(pace))

        const fastestPace = paces.toSorted()[0]

        const averagePace = paces.reduce((acc, curr) => acc + curr, 0) / paces.length
    
        return {
            name: runner.name,
            averagePace: secondsToPace(averagePace),
            fastestPace: secondsToPace(fastestPace),
        }
    }).sort((a, b) => {
        const timeA = paceToSeconds(a.averagePace)
        const timeB = paceToSeconds(b.averagePace)

        return timeA - timeB
    })

    const averagePaceAllRunners = formattedRunners.reduce((acc, curr) => {
        const averagePaceInSeconds = paceToSeconds(curr.averagePace)

        return acc + averagePaceInSeconds
    }, 0) / formattedRunners.length

    let finishIndex = 0

    formattedRunners.forEach((runner, index) => {
        const time = paceToSeconds(runner.averagePace)

        if (time <= averagePaceAllRunners) finishIndex = index
    })

    return formattedRunners.slice(0, finishIndex)
}
